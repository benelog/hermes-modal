...
- The notice banner `ll_notice` overlays the top of the chat list (message nodes under it still report their bounds); the input window `input_window_layout` overlays the bottom (list `chat_log_recycler_list` runs to the screen bottom under it).
- Photo bubble: `thumb_layout` > `image` (ImageView, cd "사진"); each bubble has a sibling `chat_info` View = the slot where the unread count / time label is drawn (no text in the tree). Both are marked not-important-for-accessibility: an a11y service sees them only with `flagIncludeNotImportantViews` (uiautomator's full dump always shows them).
- Tapping at the very bottom of the screen (gesture-nav zone, e.g. the collector app's bottom "수집 시작" button) starts a recents transition (`startExistingRecents`); an activity launched during it gets pushed back behind the previous task when the transition finishes. The a11y service may relaunch KakaoTalk itself (a bound AccessibilityService is exempt from background-activity-launch limits).
