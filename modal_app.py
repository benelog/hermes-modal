...
def _build_summary_prompt(room: str, since: str, messages: list) -> str:
    ...
    body = "\n".join(lines)
    return (
        "`kakao-room-summary` skill에 따라 아래 수집 메시지를 한국어로 요약해줘. "
        "요약문은 그대로 카카오톡 그룹 방에 전송된다. "
        f"대상 방: {room}. 기간: 최근 {since}. "
        "메시지 형식은 `[발신시각 KST] 보낸이: 내용`이며 발신시각 순으로 정렬되어 있다. "
        "공유 URL 목록은 코드가 요약 뒤에 별도로 덧붙이므로 본문에 URL 원문을 반복하지 않는다. "
        "내용이 `[사진 속 글자] …`인 메시지는 그 사람이 올린 사진(책 표지·책장·캡처 등)에서 기기가 "
        "OCR로 읽은 글자다 — 오탈자·줄 섞임이 있을 수 있으니 책 제목/저자 등을 알아보는 단서로 쓰되, "
        "확실하지 않으면 '사진으로 공유한 책(○○로 보임)'처럼 조심스럽게 표현한다.\n\n"
        f"{body}"
    )
...
def kakao_summarize(item: dict, token: str = ""):
    ...
    try:
        proc = subprocess.run(
            ["hermes", "--skills", "kakao-room-summary", "-z", prompt],
            capture_output=True,
            text=True,
            env=os.environ.copy(),
            cwd=HERMES_HOME,
