...
- Token usage, tokenmaxxing, AI inference/tooling spend, budget exhaustion, token-based pricing/monetization, input-vs-output token pricing, AI cost-management articles, and articles about employee AI-usage/token dashboards or pressure to maximize AI usage: place in `content/ai-token-cost.md`. `content/ai-trend.md` now only links to `[[ai-token-cost]]` for this topic.
- `content/ai-token-cost.md` is organized by month using H2 headings such as `## 2026년 5월`; insert each dated article into the matching month section, creating a new `## YYYY년 M월` section when needed. Keep month sections and article bullets date-descending.
...
- When adding several dated token-cost articles at once, insert each top-level bullet at its date-sorted position inside the matching month section instead of appending as a batch; run `scripts/check-ai-trend-sort.py` or an equivalent section/date-descending check afterward. For single additions, insert by date descending manually and verify the nearby ordering.
