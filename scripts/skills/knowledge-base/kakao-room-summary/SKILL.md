...
- 사용자가 "다시 확인"하거나 "오늘 수집된 건수"를 묻는 경우에는 fresh fetch를 다시 실행하고, 필요하면 `12h`, `1day`, `2day`처럼 짧은 구간도 비교한다. 결과는 `received_at`을 KST로 변환한 "오늘 수집 건수"와 `client_time[:10]` 기준 "오늘 날짜 메시지 건수"를 분리해서 보고하고, 날짜값이 빈 메시지는 별도 건수로 표시한다.
