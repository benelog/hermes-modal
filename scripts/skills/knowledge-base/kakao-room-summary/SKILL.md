...
5. 메시지가 있으면 한국어로 간결하게 요약한다. Telegram 친화적으로:
   ...
   - 대화 중 `오늘의 추천도서`로 언급된 도서명이 있으면, 본문 맨 앞에 `## 오늘의 추천도서` 섹션을 두고 도서명부터 먼저 정리한다. 저자/추천자/간단한 맥락이 확인되면 함께 적되, 확인되지 않은 정보는 지어내지 않는다.
   - 그 다음 핵심 화제 3~6개를 불릿으로, 누가 무엇을 말했는지 위주로.
