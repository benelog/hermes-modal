 ...
 * 접근성 트리 순회(PASS 1)는 여기서 하고, 좌표 결합(PASS 2)은 [FrameAssembler]가 한다.
 *
...
class KakaoCollectorService : AccessibilityService() {
    ...
    private val handledNotifs = BoundedKeySet(NOTIF_CAP)
    ...
    private fun scrape(allowTrigger: Boolean, fromScroll: Boolean) {
        ...
        for (m in frame.messages) {
            val key = DedupeKey.of(activeRoom, m.text, m.date)
        ...
        if (seekOnly || backfillCollect) {
            BackfillController.onFrame(activeRoom, BackfillController.Frame(frame.minDate, frame.signature))
        }
    ...
    /** PASS 1 — 모든 윈도우 루트를 순회해 말풍선/닉네임/날짜 마커/시각 라벨을 좌표와 함께 모은다. */
    private fun readFrameSnapshot(): FrameAssembler.Snapshot {
            ...
            walk(root) { n ->
                ...
                when {
                    nameId.isNotEmpty() && id == nameId -> {
                        ...
                        nicknames.add(SenderAssigner.NickMarker(top = rect.top, name = value))
                    }
    ...
    /**
     * 화면폭 측정. 말풍선 bounds와 '같은 좌표계'여야 내/남 정렬 판정이 맞다. 디스플레이 크기/밀도
     * override가 걸리면 displayMetrics.widthPixels가 getBoundsInScreen 좌표계와 어긋나
     * 남 메시지를 내 것으로 오판할 수 있으므로, 창 루트 bounds 폭(스크린 좌표계)을 쓴다.
     */
    private fun measureScreenWidth(roots: List<AccessibilityNodeInfo>): Int {
        var screenW = 0
        val rect = Rect()
        for (r in roots) {
            r.getBoundsInScreen(rect)
            if (rect.width() > screenW) screenW = rect.width()
        }
        return if (screenW > 0) screenW else resources.displayMetrics.widthPixels
    }
    ...
    private fun triggerSummary(room: String, command: String, deliver: (String) -> Unit) {
        ...
        ModalApi.requestSummary(room, command) { res ->
     ...
     * (대상 방/명령 본문 후보 선택은 [NotificationCommand].)
     */
    ...
    private fun handleNotification(event: AccessibilityEvent) {
        ...
        val command = NotificationCommand.parse(
            convTitle = ex(Notification.EXTRA_CONVERSATION_TITLE),
            subText = ex(Notification.EXTRA_SUB_TEXT),
            title = ex(Notification.EXTRA_TITLE),
            bigText = ex(Notification.EXTRA_BIG_TEXT),
            text = ex(Notification.EXTRA_TEXT),
            targets = Settings.roomNamesList(),
        ) ?: return

        // 같은 알림이 여러 번 와도 1회만.
        if (!handledNotifs.add(command.room + "" + command.body)) return
        ...
        val deliver: (String) -> Unit = if (replyAction != null) {
            ...
            { msg -> mainHandler.post { sendToRoom(command.room, msg) } }
        }
        triggerSummary(command.room, command.body, deliver)
    }
