...
- **이미 수집된 메시지도 다시 지나가며**, 발신 날짜/시각이 비어 있으면 제자리에서 채운다(중복 행은 안 생김).
  로컬에 완전하게 있는 메시지도 서버로 멱등 재전송한다 — 서버 보관(14일)이 폰(30일)보다 짧아
  서버에서 정리된 구간을 백필이 다시 채울 수 있게.
- **완료 시 전송 검증이 자동으로 돈다**: 미전송분을 먼저 밀어낸 뒤, 로컬 SQLite의 발신일별 건수
  (중복제거 키 기준)와 Modal `kakao-stats`의 발신일별 건수를 비교해 "전송 검증 ✅/⚠"를
  앱 상태·토스트로 알려준다. 어떤 날짜든 로컬 > 서버면 누락 의심(같은 기간 백필 재실행으로 치유),
  서버 ≥ 로컬은 정상이다.
